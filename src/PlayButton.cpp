...
PlayButton::PlayButton(Controller* game, sf::RenderWindow& window, GameManager* manager)
	:Command(game, window,manager), m_levelsMenu(m_game, m_window, m_manager)
{
	//adding buttons to the submenu
	m_levelsMenu.add(level1, std::make_unique<Level1_Button>(m_game, m_window, m_manager));
	m_levelsMenu.add(level2, std::make_unique<Level2_Button>(m_game, m_window, m_manager));
	m_levelsMenu.add(level3, std::make_unique<Level3_Button>(m_game, m_window, m_manager));
}
