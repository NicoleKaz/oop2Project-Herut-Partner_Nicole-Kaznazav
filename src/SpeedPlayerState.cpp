...
void SpeedPlayerState::move(bool* direction, b2Body* body, bool& m_touchingFloor, const b2Vec2 gravity, sf::Sprite& object)
{
    if (direction[Up])
    {
        if (m_touchingFloor)
    ...
    if (direction[Right])
    {
        ...
        body->SetLinearVelocity({ PLAYER_MOVEMENT_EXTRA_SPEED, currentSpeed_y });
    }
    if (direction[Left])
    {
        float currentSpeed_y = body->GetLinearVelocity().y;
